...
Original file is located at
    https://colab.research.google.com/drive/1Fz5-hNNWhZTiDhPKC3qONUf9IYWdD3G8
...
tmp_img = np.asarray(Image.open('PennFudanPed/PNGImages/FudanPed00001.png'))
print(tmp_img.dtype, tmp_img.shape)
plt.imshow(tmp_img)
...
tmp_mask = np.asarray(Image.open('PennFudanPed/PedMasks/FudanPed00001_mask.png'))
print(tmp_mask.dtype, tmp_mask.shape, np.unique(tmp_mask))
plt.imshow(tmp_mask)

tmp_mask[:10,:10]

tmp_mask[300:310,250:260]

tmp_mask[400:410,450:460]
...
masks = tmp_mask == obj_ids[:, None, None]
...
class PennFudanDataset(torch.utils.data.Dataset): # inherited from "data.Dataset"
    def __init__(self, root, transforms=None): # root is data directory
        self.root = root
    ...
    def __getitem__(self, idx):
        ...
        img = Image.open(img_path).convert("RGB") # convert to numpy as [h,w,channels]
        # note that we haven't converted the mask to RGB,
        # because each color corresponds to a different instance
        # with 0 being background
        mask = Image.open(mask_path)

        mask = np.array(mask) # covert to numpy as [h,w]
